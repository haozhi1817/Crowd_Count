...
from data import build_valid_dataloader_v2
...
valid_root = "/disk2/haozhi/tmp/data/test"
crop_size = 512
down_ratio = 8
batch_size = 1
norm_cood = 1
...
result_folder = '/disk2/haozhi/tmp/cc_trans/result/result_tset_910'
...
def main():
    ...
    # dataset
    valid_loader = build_valid_dataloader_v2(
        root=valid_root,
        crop_size=crop_size,
    ...
    with torch.no_grad():
        model.eval()
        for idx, (imgs, filenames) in enumerate(
            valid_loader
        ):
            imgs = imgs.to(device)
            ...
            post_process(imgs, pred_dis_maps, filenames, result_list, result_folder)
    pd.DataFrame(result_list).to_csv(os.path.join(result_folder, 'result.csv'))

            


def post_process(imgs, pred_dis_maps,filenames, result_list, save_folder):
    imgs = imgs.permute(0, 2, 3, 1).cpu().numpy()
    pred_dis_maps = pred_dis_maps.permute(0, 2, 3, 1).cpu().numpy()[...,0]
    for (img, pred_dis_map, filename) in zip(imgs, pred_dis_maps, filenames):
        img = np.array(Image.open(os.path.join(valid_root, filename + '.jpg')).convert('RGB'))
        pred_count = np.sum(pred_dis_map)
        result_list.append([filename, pred_count])
        ...
        pic = np.concatenate([img, pred_dis_map], axis = 1).astype('uint8')
        save_path = os.path.join(save_folder, filename + '_pd_' + str(pred_count) + '.jpg')
